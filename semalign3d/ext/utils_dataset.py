...
def load_pascal_data(path="data/PF-dataset-PASCAL", size=256, category='cat', split='test', subsample=None):
    ...
    for i in range(len(src_img_names)):
        src_basenmae = os.path.basename(src_img_names[i])
        trg_basenmae = os.path.basename(trg_img_names[i])
        src_fn = f'{path}/JPEGImages/{category}/{src_basenmae}'
        trg_fn = f'{path}/JPEGImages/{category}/{trg_basenmae}'
        # src_fn= f'{path}/../{src_img_names[i]}'
        # trg_fn= f'{path}/../{trg_img_names[i]}'

        # check if images exist
        if not os.path.exists(src_fn) or not os.path.exists(trg_fn):
            print(f'Image not found: {src_fn}, {os.path.exists(src_fn)} or {trg_fn}, {os.path.exists(trg_fn)}')
            continue
